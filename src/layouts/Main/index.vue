...
import VbBreadcrumbs from '@/@vb/components/Breadcrumbs/index.vue';
import VbBreadcrumbs2 from '@/@vb/components/Breadcrumbs2/index.vue';
...
export default defineComponent({
  ...
  setup() {
    ...
    const toggleMobileMenu = () => {
      ...
      settingsStore.changeSetting('isMobileMenuOpen', value);
    };

    const setViewPort = (isMobileView = false, isTabletView = false) => {
      settingsStore.changeSetting('isMobileView', isMobileView);
      settingsStore.changeSetting('isTabletView', isTabletView);
    };
    ...
    const detectViewPort = (firstLoad = false) => {
      ...
      if (state.next.tablet && !state.next.mobile && (state.next.tablet !== state.prev.tablet || firstLoad)) {
        ...
        settingsStore.changeSetting('isMenuCollapsed', true);
      }
      // mobile
