<template>
  <div
    :class="{
      ...
      [$style.menuSimply__white]: settings.menuColor === 'white',
      [$style.menuSimply__gray]: settings.menuColor === 'gray'
    }"
  >
    <div :class="$style.menuSimply__logo" class="d-none d-md-block">
      ...
      <div :class="$style.menuSimply__logo__name">{{ settings.logo }}</div>
      <div :class="$style.menuSimply__logo__descr">{{ settings.version }}</div>
    </div>
    <div class="mr-auto d-block">
      <vb-menu-pages />
    </div>
    <div class="mr-3 d-xl-block d-none">
      <vb-status />
    </div>
...
<script lang="ts">
import VbMenuPages from './MenuPages/index.vue';
