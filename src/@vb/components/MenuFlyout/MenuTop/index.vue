  ...
  <div>
    <transition-group name="air__menuFlyout__animation" :duration="1">
      <template v-for="(item, index) in renderedFlyoutItems" :key="index">
        ...
        >
          <ul :class="$style.air__menuTop__list" @mouseenter="handleFlyoutContainerOver(item.key)" @mouseleave="handleFlyoutOut(item.key)">
    ...
    >
      <div :class="$style.air__menuTop__outer">
        <a href="javascript: void(0);" :class="$style.air__menuTop__mobileToggleButton" @click="toggleMobileMenu">
          <span></span>
        </a>
    ...
    <a href="javascript: void(0);" :class="$style.air__menuTop__backdrop" @click="toggleMobileMenu"></a>
  </div>
</template>
